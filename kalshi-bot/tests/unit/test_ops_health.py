...
class MaintenanceREST:
    async def request(self, method, path, *, params=None, json=None, authenticated=True):
        ...
        if path == "/exchange/schedule":
            return {"is_maintenance": True}
        return {}


def test_fetch_exchange_health_marks_unhealthy_on_maintenance() -> None:
    health = asyncio.run(fetch_exchange_health(MaintenanceREST()))
    assert not health.healthy_for_trading
