...
def test_place_limit_order_rejects_price_above_approved_max() -> None:
    rest = FakeREST()
    manager = OrderManager(rest, SlippageGuardrails(max_slippage_dollars=Decimal("0.02")))
    decision = RiskDecision(
        approved=True,
        side="yes",
        size_fp=Decimal("10"),
        limit_price_dollars=Decimal("0.46"),
        max_price_dollars=Decimal("0.45"),
    )

    try:
        asyncio.run(manager.place_limit_order(ticker="KXTEST", decision=decision))
    except ValueError as exc:
        assert "exceeds approved max" in str(exc)
    else:
        raise AssertionError("Expected ValueError")


def test_place_limit_order_rejects_price_out_of_range() -> None:
    rest = FakeREST()
    manager = OrderManager(rest, SlippageGuardrails(max_slippage_dollars=Decimal("0.02")))
    decision = RiskDecision(
        approved=True,
        side="yes",
        size_fp=Decimal("10"),
        limit_price_dollars=Decimal("1.10"),
        max_price_dollars=Decimal("1.10"),
    )
