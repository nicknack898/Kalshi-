...
def test_normalize_signing_path() -> None:
    assert normalize_signing_path("markets?x=1") == "/markets"
...
def test_build_signature_payload_uppercases_method() -> None:
    assert build_signature_payload("1", "post", "/x") == "1POST/x"


def test_sign_request_with_direct_signing_key() -> None:
    s = sign_request(timestamp="1", method="GET", path="/markets", signing_key="secret")
